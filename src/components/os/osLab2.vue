...
<span class="token function">git</span> merge lab1
  ...
  <section type="exercise">
    <p><strong>练习 1.</strong><br>
      在文件 <code>kern/pmap.c</code> 中，实现以下函数的代码（可能按照给定的顺序）。</p>
    <ul>
      <li>boot_alloc()</li>
    ...
    <p><code>check_page_free_list()</code> 和 <code>check_page_alloc()</code> 会测试你的物理页面分配器。你应该启动 JOS 并查看 <code>check_page_alloc()</code> 是否报告成功。修改代码，并确定能够通过它的测试。你可以添加自己的 <code>assert()</code> 来验证你的假设是否正确。</p>
  </section>
  ...
  <section type="exercise">
    <p><strong>练习 2.</strong><br>
      请参阅 <a href="http://oslab.mobisys.cc/pdos.csail.mit.edu/6.828/2014/readings/i386/toc.htm">Intel 80386参考手册</a> 的第 5 章和第 6 章 。仔细阅读有关页转换和基于页的保护部分（5.2 和 6.4）。我们建议你同时浏览有关分段的部分; 虽然JOS使用的分页机制实现的虚拟内存和保护，但是段转换和段保护功能在 x86 上无法禁用，因此你需要对其有基本的了解。</p>
  </section>
  ...
  <p><img src="./lab2_1.png" alt="Lab2_1"></p>
  ...
  <section type="exercise">
    <p><strong>练习 3.</strong><br>
      虽然 GDB 只能通过虚拟地址访问 QEMU 的内存，但在设置虚拟内存的同时，查看物理内存通常很有用。从实验室工具指南里查看 QEMU 监视器命令，特别注意 xp 命令，它可以让你查看物理内存的内容。要调用 QEMU 监视器，请在终端中下 Ctrl-a c（注：这个表示先同时按下 ctrl和 a，然后再按下 c）</p>
    <p>使用 QEMU 监视器中 <code>xp</code> 命令和 GDB 中的的 <code>x</code> 命令查看相应物理地址和虚拟地址上内存的内容，并确保看到相同的数据。</p>
    <p>我们修补的 QEMU 版本提供了一个 <code>info pg</code> 的命令：它显示了当前页表的详细的表示，包括所有映射的内存范围，权限和标志。QEMU中原来还提供了一个 <code>info mem</code> 命令，显示虚拟内存的映射范围以及配置了哪些权限。</p>
  </section>
  ...
  </table><section type="question">
  <p><strong>问题 1.</strong></p>
  <p>假设以下 JOS 内核代码是正确的，变量 x 应该是什么类型？<code>uintptr_t</code> 还是 <code>physaddr_t</code>？</p>
...
x <span class="token operator">=</span>（mystery_t）value<span class="token punctuation">;</span>
  ...
  <section type="exercise">
    <p><strong>练习 4.</strong><br>
      在文件 <code>kern/pmap.c</code> 中，实现以下函数的代码。</p>
    <ul>
    ...
    <p>由 <code>mem_init()</code> 调用的 <code>check_page()</code> 可以用于测试你的页表管理程序。你应该确保测试报告成功之后再继续后面的实验。</p>
  </section>
  ...
  <section type="exercise">
    <p><strong>练习 5.</strong><br>
      补全在 <code>mem_init()</code> 函数在调用 <code>check_page()</code> 函数后的代码。</p>
    <p>你的代码现在应该能够通过 <code>check_kern_pgdir()</code> 和 <code>check_page_installed_pgdir()</code> 这两个函数的检查了。</p>
  </section>
  <section type="question">
    <p><strong>问题 2.</strong></p>
    <p>假设下图描述的是系统的页目录表，哪些条目（行）已经被填充了？它们是怎么样进行地址映射的？它们所指向的位置在哪里？请尽可能完善这张表的内容。</p>
  ...
  <section type="challenge">
    <p><strong>挑战！</strong><br>
      我们消耗了许多物理页面去保存 <code>KERNBASE</code> 的页面映射。尝试使用页面目录中的 <code>PTE_PS</code> (PS 是指 PageSize) 位以节省更多的空间。原来的 80386 并不支持此位，但是新型的 x86 处理器已经能够支持此位。因此你最好参考一下 <a href="http://oslab.mobisys.cc/pdos.csail.mit.edu/6.828/2014/readings/ia32/IA32-3A.pdf">Volume3 of the current Intel manuals</a>。确保你所设计的优化后的内核在支持它的处理器上运行。</p>
  </section>
  <section type="challenge">
    <p><strong>挑战！</strong><br>
      扩展JOS的内核监视器的命令：</p>
  ...
  <section type="challenge">
    <p><strong>挑战！</strong><br>
      写一个设计内核的大纲，使用户进程能够无限制的使用 4GB 的虚拟线性地址空间。提示：这个技术有时被称为 <code>跟随弹跳的内核</code> (follow the bouncing kernel)。在你的设计中，一定要描述清楚在内核态和用户态进行切换时到底发生了什么，内核是如何完切换的。同时也要描述出内核是在这种模式下是如何访问物理内存和 I/O 设备的。以及内核如何在系统调用期间访问用户态的虚拟地址空间等。最后，考虑并描述这种方案的灵活性，性能，内核复杂度以及其他可以考虑因素方面的优缺点。</p>
  </section>
  <section type="challenge">
    <p><strong>挑战！</strong><br>
      挑战！由于 JOS 的内核内存管理系统只能在页精细度上进行分配和释放 ，所以我们并没有任何类似 malloc/free 的功能函数。如果我们的某些类型的输入输出设备需要大于 4KB 物理地址连续的缓冲区，或者我们希望用户进程（而不仅仅是内核）能够分配和映射 4MB 的超级页面以提升处理器的效率时（看下先前挑战中关于 <code>PTE_PS</code> 的问题），这些都会成为问题。</p>
    <p>改进内核的内存分配系统，支持任何空间为 2 的整数倍的内存页大小，从 4KB 到某一个你认为合理的最大值。确保你有一些方案能够将大一点的内存单元按需求分割到一些小的分割单元，也需要能够将多个小单元重新合并为大的单元。想一想这样一个系统可能出现的问题。</p>
  </section>
