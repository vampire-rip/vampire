...
<span class="token function">git</span> merge lab3
  ...
  <p>处理器通过 映射于内存的输入输出 (memory-mapped I/O, MMIO) 来访问它的 LAPIC。在 MMIO 模式中，物理内存的一部分被硬连线于一些 I/O 设备的寄存器上（译注：I/O 设备的寄存器和内存被映射在物理内存的一些区域）， 所以通常用于访问内存的存取指令也可以同样用于访问设备寄存器。在之前的实验中，你已经在物理内存地址 0xA0000 的位置遇到过一个 IO hole（我们通过它写入 VGA 的显示缓冲区）。LAPIC 被连接在物理地址 0xFE000000 (距 4GB 还有 32MB 的位置) 的 I/O hole 上，这个地址对我们当前在 KERNBASE 上直接映射来说太高了， 不过 JOS 虚拟内存映射表 在 MMIOBASE 位置留了 4MB 的空隙，所以我们可以将这样的设备映射到这里。 之后的实验将引入更多的 MMIO 区域，因此，你将需要编写一个简单的函数，为这一区域分配内存，并将设备内存映射在上面。</p>
  <section class="custom-block exercise" type="exercise"><strong>练习 1.</strong><p>实现在 <code>kern/pmap.c</code> 中的 <code>mmio_map_region</code> 方法。你可以看看 <code>kern/lapic.c</code> 中 <code>lapic_init</code> 的开头部分，了解一下它是如何被调用的。你还需要完成接下来的练习，你的 <code>mmio_map_region</code> 才能够正常运行。</p></section>
  ...
  <section class="custom-block exercise" type="exercise"><strong>练习 2.</strong><p>阅读 <code>kern/init.c</code> 中的 <code>boot_aps()</code> 和 <code>mp_main()</code> 方法，和 <code>kern/mpentry.S</code> 中的汇编代码。确保你已经明白了引导 AP 启动的控制流执行过程。接着，修改你在 <code>kern/pmap.c</code> 中实现过的 <code>page_init()</code> 以避免将 <code>MPENTRY_PADDR</code> 加入到 free list 中，以使得我们可以安全地将 AP 的引导代码拷贝于这个物理地址并运行。你的代码应当通过我们更新过的 <code>check_page_free_list()</code> 测试，不过可能仍会在我们更新过的 <code>check_kern_pgdir()</code> 测试中失败，我们接下来将解决这个问题。</p></section>
  <section class="custom-block question" type="question"><strong>问题 1.</strong><p>逐行比较 <code>kern/mpentry.S</code> 和 <code>boot/boot.S</code>。牢记 <code>kern/mpentry.S</code> 和其他内核代码一样也是被编译和链接在 <code>KERNBASE</code> 之上运行的。那么，<code>MPBOOTPHYS</code> 这个宏定义的目的是什么呢？为什么它在 <code>kern/mpentry.S</code> 中是必要的，但在 <code>boot/boot.S</code> 却不用？换句话说，如果我们忽略掉 <code>kern/mpentry.S</code> 哪里会出现问题呢？<br>
    提示：回忆一下我们在 Lab 1 讨论的链接地址和装载地址的不同之处。</p></section>
  ...
  <section class="custom-block exercise" type="exercise"><strong>练习 3.</strong><p>修改位于 kern/pmap.c 中的 <code>mem_init_mp()</code>，将每个CPU堆栈映射在 <code>KSTACKTOP</code> 开始的区域，就像 <code>inc/memlayout.h</code> 中描述的那样。每个堆栈的大小都是 <code>KSTKSIZE</code> 字节，加上 <code>KSTKGAP</code> 字节没有被映射的 守护页 。现在，你的代码应当能够通过我们新的 <code>check_kern_pgdir()</code> 测试了。</p></section>
  <section class="custom-block exercise" type="exercise"><strong>练习 4.</strong><p>位于 <code>kern/trap.c</code> 中的 <code>trap_init_percpu()</code> 为 BSP 初始化了 TSS 和 TSS描述符，它在 Lab 3 中可以工作，但是在其他 CPU 上运行时，它是不正确的。修改这段代码使得它能够在所有 CPU 上正确执行。（注意：你的代码不应该再使用全局变量 ts。）</p></section>
...
SMP: CPU 3 starting
  ...
  <section class="custom-block exercise" type="exercise"><strong>练习 5.</strong><p>在上述提到的位置使用内核锁，加锁时使用 <code>lock_kernel()</code>， 释放锁时使用 <code>unlock_kernel()</code>。</p></section>
  <p>如何测试你的代码是正确的呢？现在还不行。你将能够在做完下一个练习，实现调度器后测试你的代码。</p>
  <section class="custom-block question" type="question"><strong>问题 2.</strong><p>看起来使用全局内核锁能够保证同一时段内只有一个 CPU 能够运行内核代码。既然这样，我们为什么还需要为每个 CPU 分配不同的内核堆栈呢？请描述一个即使我们使用了全局内核锁，共享内核堆栈仍会导致错误的情形。</p></section>
  <section class="custom-block challenge" type="challenge"><strong>挑战！</strong><p>全局内核锁很简单，而且用起来很容易。然而，它阻止了内核模式的所有可能的并发。现代操作系统使用不同的锁来保护它们共享部分中的不同部分。有一种方法叫做 fine-grained locking，这种方法可以显著地提高运行效率，但是却更难实现，而且容易出错。如果你很勇敢，抛弃全局内核锁去拥抱 JOS 中的并发吧！ 有哪些数据需要加锁保护由你来决定，作为提示，在JOS内核中你可能需要考虑在以下共享部分加自旋锁来确保排他访问：</p>
    <ul>
      <li>页面分配器(page allocator)</li>
      <li>控制台驱动器(console driver)</li>
      <li>调度器(scheduler) （译注：下一个练习实现）</li>
  ...
  <section class="custom-block exercise" type="exercise"><strong>练习 6.</strong><p>按照以上描述，实现 <code>sched_yield()</code> 轮转算法。不要忘记修改你的 <code>syscall()</code> 将相应的系统调用分发至 <code>sys_yield()</code>（译注：以后还要添加新的系统调用，同样不要忘记修改 <code>sys_yield()</code>）。</p>
...
...
    ...
    <p>如果你此时使用 <code>CPUS=1</code>，所有的用户进程应当成功运行并退出。使用超过 1 个 CPU 在没有更多的用户进程可以运行时，可能会导致发生 General Protection 或者 Kernel Page Fault，因为我们没有处理时钟中断。我们将会在下面修复这个问题。</p></section>
  <section class="custom-block question" type="question"><strong>问题 3.</strong><p>在你实现的 <code>env_run()</code> 中你应当调用了 <code>lcr3()</code>。在调用 <code>lcr3()</code> 之前和之后，你的代码应当都在引用 变量 <code>e</code>，就是 <code>env_run()</code> 所需要的参数。 在装载 <code>%cr3</code> 寄存器之后， MMU 使用的地址上下文立刻发生改变，但是处在之前地址上下文的虚拟地址（比如说 <code>e</code> ）却还能够正常工作，为什么 <code>e</code> 在地址切换前后都可以被正确地解引用呢？</p></section>
  <section class="custom-block question" type="question"><strong>问题 4.</strong><p>无论何时，内核在从一个进程切换到另一个进程时，它应当确保旧的寄存器被保存，以使得以后能够恢复。为什么？在哪里实现的呢？</p></section>
  <section class="custom-block challenge" type="challenge"><strong>挑战！</strong><p>为内核加入一个不那么平凡的调度算法吧。比如，固定优先级调度器，允许每个进程都获得一个优先级，保证拥有较高优先级的进程总是优先于低优先级进程运行。如果你觉得，这也太没挑战性了，那就试试实现一个 Unix 样式的可变优先级调度器，或者实现一个 <a href="https://en.wikipedia.org/wiki/Lottery_scheduling"><code>lottery scheduler</code></a> ，或者 <a href="https://en.wikipedia.org/wiki/Stride_scheduling"><code>stride scheduler</code></a>（你可能得谷歌一下它们是什么）。写几个测试程序，来验证你的调度算法正确工作（换句话说，正确的进程在正确的顺序被执行）。在完成这次实验的 Part B 和 Part C 的 <code>fork()</code> 函数之后，这种测试程序可能更容易写出来。</p></section>
  <section class="custom-block challenge" type="challenge"><strong>挑战！</strong><p>目前，JOS 内核还不允许应用程序使用 x86 处理器的 x87 浮点运算单元(FPU)、MMX 指令以及 流式 SIMD 扩展(SSE)。拓展 Env 结构体，来为处理器提供一个位置记录浮点运算状态，并拓展上下文切换那一部分的代码，以在进程切换时正确保存这个状态。<code>FXSAVE</code> 和 <code>FSRSTR</code> 这两个指令在这里可能非常有用，但是因为它们是在近期才被引入的，所以可能不会出现在旧的 i386 用户手册中。写一个用户模式的测试程序，进行浮点运算。这样是不是很酷？</p></section>
  ...
  <section class="custom-block exercise" type="exercise"><strong>练习 7.</strong><p>在 <code>kern/syscall.c</code> 中实现上面描述的系统调用。你将需要用到在 <code>kern/pmap.c</code> 和 <code>kern/env.c</code> 中定义的多个函数，尤其是 <code>envid2env()</code>。此时，无论何时你调用 <code>envid2env()</code>，都应该传递 1 给 <code>checkperm</code> 参数。确定你检查了每个系统调用参数均合法，否则返回 <code>-E_INVAL</code>。 用 <code>user/dumbfork</code> 来测试你的 JOS 内核，在继续前确定它正常的工作。（<code>make run-dumbfork</code>）</p></section>
  <section class="custom-block challenge" type="challenge"><strong>挑战！</strong><p>添加一些额外的系统调用，来读取并设置一个已有进程的全部状态。然后，再来实现一个用户模式进程，它派生一个子进程，让子进程运行一会儿（比如，调用几次 <code>sys_yield()</code>），然后为子进程做一个完整的快照（检查点）。再让子进程运行一会儿，然后，将子进程的所有状态还原到之前的检查点，并让子进程从检查点继续运行。这样，你就实现了从一个状态 “重放” 子进程的执行过程。让子进程进行一些类似 <code>sys_cgetc()</code> 或 <code>readline()</code> 这种和用户交互的操作，以确保能够观测和修改它的执行状态，并确保你的 检查点/重放 功能能够让子进程 “选择性失忆”，忘掉它在某个特定时间后做的全部事情。</p></section>
  ...
  <section class="custom-block exercise" type="exercise"><strong>练习 8.</strong><p>实现 <code>sys_env_set_pgfault_upcall</code> 系统调用。因为这是一个 “危险” 的系统调用，不要忘记在获得目标进程信息时启用权限检查。</p></section>
...
（译注：这个结构比在代码注释中提到的结构更靠谱一点）
  ...
  <section class="custom-block exercise" type="exercise"><strong>练习 9.</strong><p>实现在 <code>kern/trap.c</code> 中的 <code>page_fault_handler</code> 方法，使其能够将缺页分发给用户模式缺页处理函数。确认你在写入异常堆栈时已经采取足够的预防措施了。（如果用户进程的异常堆栈已经没有空间了会发生什么？）</p></section>
  ...
  <section class="custom-block exercise" type="exercise"><strong>练习 10.</strong><p>实现在 <code>lib/pfentry.S</code> 中的 <code>_pgfault_upcall</code> 例程。返回到一开始运行造成缺页的用户代码这一部分很有趣。你在这里将会直接返回，而不是通过内核。最难的部分是同时调整堆栈并重新装载 EIP。</p></section>
  <p>最终，你需要实现 C 用户调用库这边的用户模式缺页处理机制。</p>
  <section class="custom-block exercise" type="exercise"><strong>练习 11.</strong><p>完成在 <code>lib/pgfault.c</code> 中的 <code>set_pgfault_handler()</code> 。</p></section>
  <h4 id="测试">测试</h4>
...
[00001000] free env 00001000
  ...
  <section class="custom-block challenge" type="challenge"><strong>挑战！</strong><p>拓展你的内核，让所有用户模式代码产生的处理器异常（而不仅仅是缺页），都可以分发给用户模式异常处理函数来处理。写一个测试程序来测试一下不同用户模式异常的处理是否能正常工作，比如，除零，一般保护错，或者错误的操作码。</p></section>
  ...
  <section class="custom-block exercise" type="exercise"><strong>练习 12.</strong><p>实现在 <code>lib/fork.c</code> 中的 <code>fork</code>，<code>duppage</code> 和 <code>pgfault</code>。 用 <code>forktree</code> 程序来测试你的代码(<code>make run-forktree</code>)。它应当产生下面的输出，其中夹杂着 <strong>new env</strong>, <strong>free env</strong> 和 <strong>exiting gracefully</strong> 这样的消息。下面的这些输出可能不是按照顺序的，进程ID也可能有所不同：</p>
    <pre><code>1000: I am ''
1001: I am '0'
2000: I am '00'
2001: I am '000'
...
1006: I am '101'
  ...
  <section class="custom-block challenge" type="challenge"><strong>挑战！</strong><p>实现一个共享内存的派生，我们叫它 <code>sfork()</code> 好了。这个版本的派生应该让父进程和子进程共享他们所有的内存页面（所以，在一个进程中写内存，在另一个进程中也会反映出来），不过栈这一页所使用的内存区域例外，这一区域应该被当做 copy-on-write 的。修改 <code>user/forktree.c</code> 来使用新的 <code>sfork()</code> 而不是以前的 <code>fork()</code>。当你在 Part C 完成了 IPC 之后，再试试用 <code>sfork()</code> 来运行 <code>user/pingpongs</code>。你大概必须找一种新的方式才能使 <code>thisenv</code> 这个全局指针正常工作了。</p></section>
  <section class="custom-block challenge" type="challenge"><strong>挑战！</strong><p>你实现的 fork 也许调用了太多的系统调用。在 x86 中，通过中断在内核和用户模式中切换的代价可不小。试着改一改系统调用接口，让它能一次性地做很多很多系统调用。接下来，修改一下我们的 fork()，让它来使用这个新的接口。测一测，通过你新实现的 fork，到底快了多少。你可以粗略地通过算一算调用 int 0x30 需要多长时间、你的新实现少调用了多少 int 0x30、访问 TSS 栈是不是也很耗时等等这些类似的方法来回答这个问题。另外，你也可以在真正的硬件上跑一跑你的代码，做做评测。可以参考一下 IA32 手册上的 RDTSC 指令，这一指令用来统计自上次处理器重置时消耗的时钟周期数目。QEMU 不会帮我们好好做这件事，它可能会用执行了多少虚拟指令或者主机上的 TSC 来糊弄我们，这些都不能反映到底需要多少的 CPU 时钟周期。</p></section>
  ...
  <section class="custom-block exercise" type="exercise"><strong>练习 13.</strong><p>修改 <code>kern/trapenrty.S</code> 和 <code>kern/trap.c</code> 来初始化一个合适的 IDT 入口，并为 IRQ 0-15 提供处理函数。接着，修改 <code>kern/env.c</code> 中的<code>env_alloc()</code> 以确保用户进程总是在中断被打开的情况下运行。</p>
    ...
    <p>完成这个练习后，当你运行任何一个运行时间较长的测试程序时（比如 <code>make run-spin</code>），你应当看见内核打印硬件中断的 trap frame。因为到目前为止，虽然处理器的硬件中断已经被打开了，但 JOS 还没有处理它，所以你应该注意到，它以为这个中断发生在正在运行的用户进程，并将其销毁。最终当没有进程可以销毁的时候，JOS 会陷入内核监视器。</p></section>
  ...
  <section class="custom-block exercise" type="exercise"><strong>练习 14.</strong><p>修改内核的 <code>trap_dispatch()</code> 函数，使得其每当收到时钟中断的时候，它会调用 <code>sched_yield()</code> 寻找另一个进程并运行。</p>
    <p>你现在应当能让 <code>user/spin</code> 测试程序正常工作了（译注：这里有一个文档中没有提到的细节。如果你发现时钟中断只发生一次就再也不会发生了，你应当再去看看 <code>kern/lapic.c</code>）：父进程会创建子进程，<code>sys_yield()</code> 会切换到子进程几次，但在时间片过后父进程会重新占据 CPU，并最终杀死子进程并正常退出。</p></section>
  ...
  <section class="custom-block exercise" type="exercise"><strong>练习 15.</strong><p>实现 <code>kern/syscall.c</code> 中的 <code>sys_ipc_recv</code> 和 <code>sys_ipc_try_send</code>。在实现它们前，你应当读读两边的注释，因为它们需要协同工作。当你在这些例程中调用 <code>envid2env</code> 时，你应当将 <code>checkperm</code> 设置为 0，这意味着进程可以与任何其他进程通信，内核除了确保目标进程 ID 有效之外，不会做其他任何检查。</p>
    ...
    <p><code>user/primes.c</code> 用来创建子进程和通信的代码读起来可能很有趣。（译注：可能因为 <code>user/primes</code> 的输出过多，有时无法从 QEMU 输出串口读取全部的输出，测试脚本可能判定程序运行错误。多运行几次试试看？）</p></section>
  <section class="custom-block challenge" type="challenge"><strong>挑战！</strong><p>为什么 <code>ipc_send</code> 必须要循环？调整我们的系统调用接口，让它没必要循环。确保你的实现能够正确处理多个进程同时向一个进程发消息的情况。</p></section>
  <section class="custom-block challenge" type="challenge"><strong>挑战！</strong><p>素数筛这个程序只是多进程间通信的一个魔法用法。读一读 <code>C. A. R. Hoare, Communicating Sequential Processes, Communications of the ACM 21(8) (August 1978), 666-667</code>，试试实现例子中的矩阵乘法。（译注：<a href="http://cs2.ist.unomaha.edu/~stanw/papers/p666-hoare.pdf">链接在这里</a>，矩阵乘法在 6.2 这个小结）</p></section>
  <section class="custom-block challenge" type="challenge"><strong>挑战！</strong><p>最令人惊奇的利用通信来传递信息的例子之一应该就属 Doug McIlroy 的幂计算器了，在<a href="https://swtch.com/~rsc/thread/squint.pdf">M. Douglas McIlroy,``Squinting at Power Series,’’ Software–Practice and Experience, 20(7) (July 1990), 661-683</a> 这里。实现里面介绍的幂计算器，并算算 <code>sin(x+x^3)</code></p></section>
  <section class="custom-block challenge" type="challenge"><strong>挑战！</strong><p>通过实现 Liedtke 论文中的一些技术，让我们 JOS 的 IPC 机制更快一些吧。文章在这里， <a href="http://dl.acm.org/citation.cfm?id=168633">Improving IPC by Kernel Design</a>。如果你觉得你还有别的什么更好的办法，也试试看？总之，请随意修改内核的系统调用 API 吧，只要还能够通过我们的打分脚本就行。</p></section>
