...
<div :id="$options.name" :class="$options.name" class="content">
  <h1 id="lab-1-booting-a-pc--启动pc">Lab 1: Booting a PC / 启动PC</h1>
  ...
  <p>你需要根据我们的 <strong>环境搭建指南</strong> 来安装 <code>qemu</code> 和 <code>gcc</code> 。我们对 <code>qemu</code> 进行过更改，加入了一些有用的调试功能，接下来的一些实验会用到这些修改的补丁，需要实验依赖的补丁因此，因此，你必须在你的机器上编译和安装我们修改过的 <code>qemu</code>，而不是下载 <code>qemu</code> 的发行版。<s>如果你的机器用来自己的 ELF 的工具链（例如 Linux 和绝大多数的 BSD 的，但不是 OSX）你需要从你的安装包中安装一下 gcc 编译器，你可以按照工具页面上的说明进行操作。</s>（不适用于我们的实验情况）</p>
  <h3 id="hand-in-procedure--作业提交过程">Hand-In Procedure / 作业提交过程</h3>
  <p>我们会通过一个评定程序对你的解决方案进行一个评定。你可以通过评定程序并运行 <code>make grade</code> 来检测的你的解决方案。</p>
  <p>不必运行 <code>make handin</code> 等其他文档中的命令，那些将会触发向 MIT 服务器的提交工作，对我们来说是无效的。也不要使用 <code>git pull</code>，<code>git fetch</code> 等操作。</p>
  <h2 id="part-1-pc-bootstrap--pc引导程序">Part 1: PC Bootstrap / PC引导程序</h2>
  <p>第一个练习的目的是向你介绍有关 x86 汇编编程语言和 PC 的引导进程，并让你了解 QEMU 和 QEMU/GDB 的调试。接下来你并不需要为这个实验写任何的代码，但是为了你能更好的理解这个实验，你必须认真浏览一下。</p>
  ...
  <p>在实验中我们将使用 <a href="http://www.qemu.org/">QEMU 模拟器</a>，一个更现代更快的模拟器。虽然 QEMU 内置了显示器仅提供了有限的调试功能，QEMU 可以当作 GNU 调试器的远程调试目标(GDB)，我们将在实验中使用这种方式，来完成早期的启动过程。</p>
  <p>开始前，我们解压 <code>lab.zip</code> 中的文件到你的主机里，接下来在 <code>lab</code> 文件夹下输入 <code>make</code> 去构建最小的加载启动程序和内核。（现阶段把这些代码叫作 <strong>内核</strong> 确实有些名不符实，不过接下来的学期中我们将逐步的完善这份代码。）</p>
  <pre class=" language-bash"><code class="prism  language-bash"><span class="token function">cd</span> lab
...
+ cc -Os boot/main.c
+ ld boot/boot
boot block is 414 bytes <span class="token punctuation">(</span>max 510<span class="token punctuation">)</span>
+ mk obj/kern/kernel.img
</code></pre>
  <p>现在，你将要以上述过程编译出的 <code>obj/kern/kernel.img</code> 作为虚拟硬盘来运行 QEMU。这个硬盘镜像包含了我们需要的启动代码 bootloader（<code>obj/boot/boot</code>） 以及我们使用的内核（<code>obj/kernel</code>）</p>
  <pre class=" language-bash"><code class="prism  language-bash"><span class="token function">make</span> qemu
<span class="token comment"># 对于 WSL 等，请使用↓</span>
<span class="token function">make</span> qemu-nox
</code></pre>
  <p>这条命令将执行 QEMU，默认的配置是使用编译出来的镜像作为硬盘，并将串行端口输出显示到终端上。某些文本应显示在QEMU窗口中：</p>
  <pre class=" language-jos"><code class="prism  language-jos">Booting from Hard Disk...
...
K&gt;
  ...
  <p>从 <code>Booting from Hard Disk…</code> 后所有内容都是从我们的 JOS 内核打印出来的； <code>K&gt;</code> 是型监视器提示符，是一个交互式控制程序，已经包含在内核中了。以上这些行也会出现在运行 QEMU 的常规 shell 窗口中。为了实现自动测试和评分，我们调整了 JOS 的内核将其控制结果的输出不仅写到了虚拟的 VGA 显示器中（能在 QEMU 窗口上看见的，用 WSL 的话，无法看见这个窗口），也输入到模拟 PC 的虚拟串行接口上，QEMU 将输出转为自己的标准输出，显示在屏幕上。同样的，JOS 内核也会接收来自于键盘和其它串行设备的输入，因此你既可以将指令输入到 VGA 的显示器上或输入到运行 QEMU 的终端上。另外，你也可以运行 <code>make qemu-nox</code> 使用没有 VGA 显示器的串行控制台。 如果你是用的 WSL，或是 SSH 远程登录的方式连接在服务器上进行实验，这可能很方便。</p>
  <p>现在内核监视器只支持两种命令，<code>help</code> 和 <code>kerninfo</code>。试试看。</p>
  <p><code>help</code> 命令的作用很明显，我们接下来将简单讨论 <code>kerninfo</code> 命令打印内容的含义。虽然简单，但是需要注意的是，内核监视器是 <strong>直接</strong> 在 PC 模拟器的 <strong>原生（虚拟）的硬件</strong> 上运行的。 这意味着如果你将 <code>obj/kern/kernel.img</code> 的内容全部拷贝到硬盘的前几个扇区，将硬盘插入到你的实际的 PC 机器上，打开它，你会发现在你 PC 屏幕上的内容和你在 QEMU 窗口上所看到的内容将会是一样的。（我们不建议你用电脑来操作，一旦你将 <code>kernel.img</code> 的内容拷贝到你硬盘修改主引导记录，它会覆盖掉第一扇区的开头部分，这样硬盘上所有的东西都会丢失！）</p>
  <h3 id="pc-的物理地址空间">PC 的物理地址空间</h3>
  <p>现在我们将描述一下PC的启动过程。一个PC的物理地址空间具有以下大致的布局。</p>
  <p><img src="./lab1_1.png" alt="lab1_1.png"></p>
  <p>第一代PC是基于 16 位的 Intel8088 处理器设计的，只能处理 1MB 的物理内存。因此早期的 PC的物理地址空间都是以 <code>0x00000000</code> 开始，但不是以现在 <code>0xFFFFFFFF</code> 结束的而是 <code>0x000FFFFF</code>。前 <code>640KB</code> 区域被标记为 <strong>低地址空间</strong>，是早期 PC 唯一可以使用的随机访问存储区（RAM），事实上，早期的 PC 只配有 16KB, 32KB, 或者 64KB 的 RAM！</p>
  <p>从 <code>0x000A0000</code> 到 <code>0x000FFFFF</code> 的 384KB 区域被保留下来用作硬件的一些特殊用途，比如视频播放的缓冲区和保存固件的非易失性内存空间。这一部分保留区域中最重要的一部分是基本的输入输出系统（简称BIOS）,占据了从 <code>0x000F0000</code> 到 <code>0x000FFFFF</code> 这 <code>64KB</code> 的区域。早期的 PC 机 BIOS 保存在只读内存区域，现在的 PC 机上的 BIOS 存储在新型的闪存存储器上。BIOS 负责执行系统的基本初始化，例如激活视频卡或者是检查内存安装的总量。在执行完初始化操作后，BIOS 将从一些合适的地方加载操作系统，例如软盘，硬盘，光盘，或者网络上，然后将机器的控制权交给操作系统。</p>
  <p>英特尔的 80286 和 80386 处理器最终打破了 <strong>一兆字节的瓶颈</strong>，可以支持 16MB 和 4GB 的物理地址空间，可是 PC 的架构师依然要保留原来 1MB 的低地址的物理空间布局以保证现有软件的向下兼容性。因此，现代 PC 在 <code>0x000A0000</code> 到 <code>0x00100000</code> 的物理地址上有一部分 <strong>空洞</strong>，将RAM划分为两段，<strong>低地址内存空间</strong>（第一个 640KB 的内存空间）和 <strong>扩展内存空间</strong>（其余的部分）。此外，一些在 32 位机器的物理地址空间（最重要的是物理 RAM）最顶端的一些空间，通常被 BIOS 保存用作 32 位的 PCI 的设备使用。</p>
  <p>最近 x86 的处理器能支持超过 4GB 的物理内存，因此 RAM 可以拓展到 <code>0xFFFFFFFF</code> 之上。在这种情况下 BIOS 必须要在 32 位系统内存高层预留第二个 <strong>空洞</strong>，用于为一些 32 位的设备预留映射空间。由于设计的限制，JOS只能使用前 265MB 的物理内存，现在我们假设电脑仅有一个连续的 32 位的物理地址空间。对于复杂的物理空间结构的管理和多种多样的已有设备的兼容，一直是 OS 不得不面对的一个问题，也是是未来发展中会一直存在的挑战。</p>
  <h3 id="只读存储器-bios">只读存储器 BIOS</h3>
  <p>在这一部分实验中，你将使用 QEMU 的调试功能来检查一个 IA-32 兼容的计算机的引导过程。</p>
  <p>打开两个终端窗口，在一个里面输入 <code>make qemu-gdb</code> (或者是 <code>make qemu-nox-gdb</code> )。这个将启动 QEMU，但是在 QEMU 将在处理器执行第一条指令前停止，并等待来自 GDB 的调试连接。在第二个终端窗口，在运行 make 那个目录运行 gdb，你可以看到像这样的一些信息：</p>
  <pre class=" language-gdb"><code class="prism  language-gdb">The target architecture is assumed to be i8086
...
(gdb)
  ...
  <p>这是 GDB 对要执行的第一条指令拆解。从这个输出我们可以总结出以下几件事情：</p>
  <ul>
    <li>IBM 的 PC 是从物理地址是 0x000FFFF0 的内存位置开始执行的，这是为只读存储器 BIOS 保留的 64KB 的区域的最顶端。PC开始执行时 CS = 0xF000 和 IP = 0xFFF0</li>
    <li>第一条被执行的指令是 jmp 指令，跳转到分区地址是 CS = 0xF000 和 IP = 0x305B</li>
  </ul>
  <p>为什么 QEMU 是这样开始的？这是完全按照英特尔设计的 8088 处理器工作的，也就是模拟了 IBM的 PC 机上的处理器工作模式。因为在 PC 中， BIOS 是通过 <strong>硬件接线</strong> 的方式连接到物理地址 <code>0x000F0000</code> - <code>0x000FFFFF</code> 这段范围上，这种设计能确保 BIOS 在电脑后充电后开机或者是任何操作系统重启后立刻获取到机器的控制权。这十分的重要，因为重启后 RAM 中不会有任何的软件可供处理器执行。QEMU 模拟器有自己的 BIOS，它位于处理器模拟的物理地址空间的位置上。在处理器复位的时候，（模拟）处理器进入实模式将 CS 设置到 <code>0xF000</code> 并且将 IP 设置到 <code>0xFFF0</code> ,以便从（CS：IP）地址开始执行。分段机制地址 <code>0xF000:FFF0</code> 如何转为物理地址的呢？</p>
  <p>为了回答这个问题，我们必须了解一些实际模式寻址的知识。在实际模式（PC 的启动模式），地址翻译是根据公式： 物理地址 =  16 * 段 + 偏移。因此。当将 PC 的 CS 设置为 0xf000 IP 设置为 0xfff0，物理地址就是：</p>
  <pre class=" language-magic"><code class="prism  language-magic">16 * 0xf000 + 0xfff0   # 在 16 进制下乘以 16
  ...
  <p>PC 的软盘和硬盘以 512 个字节为单位划分成区域，每个区域称为一个扇区。扇区是磁盘的最小访问单位：每个读、写操作的大小必须是一个或多个扇区，并在扇区边界上对齐。如果磁盘是可引导的，则第一个扇区称为引导扇区，因为这是引导加载程序代码所在的位置。当 BIOS 找到可启动的软盘或硬盘时，将 512 字节的引导扇区加载到物理地址 0x7C00 到 0x7DFF 的内存中，然后使用 jmp 指令将 <code>CS:IP</code> 设置为 <code>0000:7C00</code>，将控制权传递给引导程序。引导程序又称为 bootloader。像 BIOS 加载地址一样，这些地址是随意设定的 —— 但对于 PC 来说，它们是固定和标准化的。</p>
  <p>在 PC 的演进过程中，从 CD-ROM 启动的能力要晚很多才出现，因此，PC 架构师借此机会重新思考引导过程。因此，现代 BIOS 从 CD-ROM 启动的方式有点复杂（且更强大）。CD-ROM 使用 2048 字节大小的扇区，而不是 512 字节，并且在将控制转移给它之前，BIOS 可以将更大的启动映像从磁盘加载到内存（而不仅仅是一个扇区）中。有关详细信息，请参阅 <a href="http://oslab.mobisys.cc/pdos.csail.mit.edu/6.828/2014/readings/boot-cdrom.pdf">“El Torito” Bootable CD-ROM Format Specification</a>。但是，在我们的实验中，我们将使用传统的硬盘启动机制，这意味着我们的引导加载程序必须放进 512 字节的区域中。引导加载程序由两个文件组成，一个汇编语言源文件 <code>boot/boot.S</code> 和一个 C 语言源文件 <code>boot/main.c</code>。仔细阅读这些源文件，并确保你了解发生了什么。引导加载程序必须执行两个主要功能：</p>
  <p>首先，引导加载程序将处理器从实模式切换到 32 位保护模式，因为只有在该模式下，软件才能访问处理器物理地址空间中 1MB 以上的所有内存。保护模式在 <a href="http://oslab.mobisys.cc/pdos.csail.mit.edu/6.828/2014/readings/pcasm-book.pdf">PC Assembly Language</a> 第 1.2.7 和 1.2.8 节中有简要介绍，并在英特尔架构手册中有详细介绍。在这个阶段上，你只需要理解，在保护模式下，将分段地址（段地址：偏移量的组合）转换为物理地址的方式不同，而转换后的偏移量为 32 位而不是 16 位。</p>
  <p>其次，引导加载程序通过 x86 特殊的 I/O 指令直接访问 IDE 磁盘设备寄存器，从硬盘读取内核。如果你想更好地了解这里的 <em>特殊 I/O 指令</em> 是什么意思，请查看 <a href="http://oslab.mobisys.cc/pdos.csail.mit.edu/6.828/2014/reference.html">6.828 reference page</a> 的 IDE 硬盘驱动器控制器 <code>IDE hard drive controller</code> 部分。在本课程中, 你不需要了解编程特定设备的情况：编写设备驱动程序实际上是操作系统开发中非常重要的一部分，但从概念或架构的角度来看，它也是最无趣的一部分。</p>
  ...
  <ul>
    ...
    <li>引导加载程序 bootloader 执行的最后一个指令是什么，加载的内核的第一个指令是什么？</li>
    <li>内核的第一个指令在哪里？</li>
    <li>引导加载程序如何决定为了从磁盘获取整个内核必须读取多少扇区？在哪里可以找到这些信息？</li>
  </ul>
  <h3 id="加载内核">加载内核</h3>
  ...
  <p>ELF 二进制文件以固定长度的 ELF 头开始，后面是一个可变长度的程序头，程序头中列出了要加载的每个程序段。这些 ELF 头的 C 定义在 inc/elf.h 中。我们感兴趣的程序部分是：</p>
  <ul>
    <li><code>.text</code>：程序的可执行指令。</li>
    <li><code>.rodata</code>：只读数据，如 C 编译器生成的 ASCII 字符串常量（虽说是只读，我们并不会专门设置硬件来禁止写操作）。</li>
    <li><code>.data</code>：数据部分保存程序的初始化数据，例如用 <code>int x = 5</code> 初始化时声明的全局变量。</li>
  </ul>
  <p>当链接器计算程序的存储器布局时，它会为未初始化的全局变量保留空间（例如 <code>int x</code> 这种变量），这些变量会放在一个名为 <code>.bss</code> 的段中，紧跟着 <code>.data</code> 的后面。C 要求 <code>未初始化</code> 的全局变量自动被初始化为 <code>0</code>，因此，不需要存储 <code>.bss</code> 部分。链接器只记录了 <code>.bss</code> 段的地址和大小。加载程序或程序本身必须将 <code>.bss</code> 部分置零。</p>
  <p>通过键入以下内容，检查内核可执行文件中所有部分的名称，大小和链接地址的完整列表：</p>
  <pre class=" language-bash"><code class="prism  language-bash">objdump -h obj/kern/kernel
</code></pre>
  <p>你会看到比上面列出的更多部分，但其他部分对我们的目的来说并不重要。其他的大多数都是保存调试信息，这通常包含在程序的可执行文件中，但不由加载程序加载到内存中。</p>
  <p>特别注意 <code>.text</code> 部分的 <code>VMA</code>（或者说链接地址 <code>link address</code> ）和 <code>LMA</code>（或者说加载地址 <code>load address</code>） 。每部分的加载地址是将该部分加载到内存中的内存地址。</p>
  <p>段的链接地址是该段期望执行的内存地址。链接器在生成的二进制程序中大量使用链接生成的绝对地址，例如当代码中需要某个全局变量的地址时会直接使用由链接器生成的变量的链接地址，但这样做的结果是，如果二进制文件没有被加载到链接地址，而是从一个地址开始执行时，它会产生很多链接地址错误。（生成不包含任何这样的绝对地址的位置无关代码也是可以做到的，这种技术在现代共享库中广泛使用，但它当然也会引入性能和复杂性的代价，所以我们不会在本实验中使用它）。</p>
  <p>通常，链接和加载地址是相同的。例如，查看引导加载程序的 <code>.text</code> 部分：</p>
  <pre class=" language-bash"><code class="prism  language-bash">objdump -h obj/boot/boot.out
</code></pre>
  <p>bootloader 程序使用 ELF 程序头来决定如何加载这部分。程序头指定要加载到内存中的 ELF 对象的段的位置以及每个段应该占用的目标地址。你可以通过键入以下内容检查内核的程序头：</p>
  <pre class=" language-bash"><code class="prism  language-bash">objdump -x obj/kern/kernel
</code></pre>
  ...
  <p>当你检查上面的 bootloader 程序的链接和加载地址时，它们是完全匹配的，但内核的链接地址（由 <code>objdump</code> 打印）与其加载地址之间存在一个（相当大的）差异。回去检查这两个概念，确保你可以看懂我们在说什么。 （链接内核比引导加载程序更复杂，链接和加载地址位于 <code>kern/kernel.ld</code> 的顶部。）</p>
  <p>操作系统内核通常喜欢链接并运行在非常高的虚拟地址（例如 <code>0xf0100000</code> ），以便留下处理器虚拟地址空间的较低部分供用户程序使用。 在下一个实验中，这种安排的原因将变得更加清晰。</p>
  ...
  <ul>
    <li>
      <p>解释 <code>printf.c</code> 和 <code>console.c</code> 之间的接口。具体来说，<code>console.c</code> 导出了什么函数？<code>printf.c</code> 是如何使用这些函数的？</p>
    </li>
    <li>
      <p>解释 <code>console.c</code> 的以下内容：</p>
      <p><img src="./lab1_2.png" alt="Lab1_2"></p>
    </li>
    <li>
      <p>对于以下问题，你可能需要参考一些课外资料。</p>
      <p>跟踪以下代码的并单步执行：</p>
      <pre class=" language-c"><code class="prism  language-c"><span class="token keyword">int</span> x <span class="token operator">=</span> <span class="token number">1</span><span class="token punctuation">,</span> y <span class="token operator">=</span> <span class="token number">3</span><span class="token punctuation">,</span> z <span class="token operator">=</span> <span class="token number">4</span><span class="token punctuation">;</span>
<span class="token function">cprintf</span><span class="token punctuation">(</span><span class="token string">"x %d, y %x, z %d\n"</span><span class="token punctuation">,</span> x<span class="token punctuation">,</span> y<span class="token punctuation">,</span> z<span class="token punctuation">)</span><span class="token punctuation">;</span>
      ...
      <ul>
        <li>在调用 <code>cprintf()</code> 时，<code>fmt</code> 是什么意思？ <code>ap</code> 是什么意思？</li>
        <li>按执行顺序列出每次调用 <code>cons_putc</code>，<code>va_arg</code> 和 <code>vcprintf</code> 这三段代码时的细节。 对于 <code>cons_putc</code>，列出其参数。 对于 <code>va_arg</code>，列出调用之前和之后的 <code>ap</code> 指针的指向。 对于 <code>vcprintf</code>，列出其两个参数的值。</li>
      </ul>
    </li>
    <li>
      <p>运行下面的代码</p>
      <pre class=" language-c"><code class="prism  language-c"><span class="token keyword">unsigned</span> <span class="token keyword">int</span> i <span class="token operator">=</span> <span class="token number">0x00646c72</span><span class="token punctuation">;</span>
<span class="token function">cprintf</span><span class="token punctuation">(</span><span class="token string">"H%x Wo%s"</span><span class="token punctuation">,</span> <span class="token number">57616</span><span class="token punctuation">,</span> <span class="token operator">&amp;</span>i<span class="token punctuation">)</span><span class="token punctuation">;</span>
      ...
      <p>输出是什么？如何按照上一个练习的执行步骤，说明为什么会显示这个输出信息。可以参考将字节映射到字符的 <a href="http://web.cs.mun.ca/~michael/c/ascii-table.html">ASCII表</a>。</p>
      <p>由于 x86 是小端的，所以得到了上面的输出结果。如果 x86 是大端，那么为了产生相同的输出，你会设置什么？你需要将 57616 更改为不同的值吗？<a href="http://www.webopedia.com/TERM/b/big_endian.html">这里</a>是一个关于大小端的描述，<a href="http://www.networksorcery.com/enp/ien/ien137.txt">这里</a>是一个更加脑洞大开的说法。</p>
    </li>
    <li>
      <p>在下面的代码中，将在 <code>y =</code> 之后打印什么（注意：答案不是一个固定的值）？为什么会发生这种情况？</p>
      <pre class=" language-c"><code class="prism  language-c"><span class="token function">cprintf</span><span class="token punctuation">(</span><span class="token string">"x=%d y=%d"</span><span class="token punctuation">,</span> <span class="token number">3</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre>
    </li>
    <li>
  ...
  <p>打出的第一行反映了当前正在执行的函数，即 <code>mon_backtrace</code> 本身，第二行反映的是调用 <code>mon_backtrace</code> 的函数，第三行反映了调用该函数的函数，依此类推。 你应该打印所有的堆栈帧。通过了解 <code>kern/entry.S</code>，你会发现有一个简单的方法来告诉你这个链条的回溯应该到什么时候停止。</p>
  <p>每行包含一个 <code>ebp</code>，<code>eip</code> 和 <code>args</code>。<code>ebp</code> 的值表示的是该函数在栈中的基地址，即刚刚进入到函数的代码时栈指针的位置，在函数的前导代码中完成这个指针的设置。列出的 <code>eip</code> 值是函数的返回指令指针，即当函数返回时控制流会返回到的指令地址。返回指令指针通常指向 <code>call</code> 指令后的指令（为什么？）。 最后，<code>args</code> 之后列出的 <code>5</code> 个十六进制值是当前函数的前 5 个参数，这些参数会在函数调用之前被压入到栈中。 如果函数被调用的参数少于 5 个，那么当然并不是所有这 5 个值都是有意义的（为什么回溯代码不能检测到实际有多少个参数？用什么方法可以弥补这个缺陷？）。</p>
  <p>以下是你在 <a href="http://video.mobisys.cc/materials/tools/CProgrammingLanguageCN.pdf">K＆R 的书</a> 第 5 章的阅读时应注意到的几个具体要点，为了接下来的联系和以后的实验，你应该记下来。</p>
  <ul>
    <li>如果 <code>int *p =（int *）100</code> ，则<code>(int)p + 1</code> 和 <code>(int)(p + 1)</code> 是不一样的：第一个是 <code>101</code>，但第二个是 <code>104</code>。当向指针加上一个整数时， 在第二种情况下，整数默认会乘以指针指向的对象的大小。</li>
    <li><code>p[i]</code> 的定义与 <code>*(p + i)</code> 相同，指的是 <code>p</code> 指向的存储器中的第 <code>i</code> 个对象。上述的规则在对象大于一个字节时是非常需要注意的。</li>
  ...
  <section class="custom-block exercise" type="exercise"><strong>练习 12.</strong><p>修改你的堆栈回溯功能，为每个 <code>eip</code> 显示与该 <code>eip</code> 对应的函数名称，源文件名和行号。</p>
    ...
    <ul>
      ...
      <li>运行 <code>gcc -pipe -nostdinc -O2 -fno-builtin -I. -MD -Wall -Wno-format -DJOS_KERNEL -gstabs -c -S kern/init.c</code>，并查看 <code>init.s</code></li>
      <li>看看 <code>bootloader</code> 是否在内存中加载了符号表，作为加载内核二进制文件的一部分</li>
    </ul>
    <p>通过插入对 <code>stab_binsearch</code> 的调用来查找地址的行号，补全 <code>debuginfo_eip</code> 的实现。</p>
    <p>向内核监视器添加一个 <code>backtrace</code> 命令，并扩展你的 <code>mon_backtrace</code> 的实现，调用 <code>debuginfo_eip</code> 并为每个堆栈框架打印一行：</p>
    <pre class=" language-jos"><code class="prism  language-jos">K&gt; backtrace
Stack backtrace:
