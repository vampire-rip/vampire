...
<span class="token function">git</span> merge lab2
  ...
  <section class="custom-block exercise" type="exercise"><strong>练习 1.</strong><p>修改 <code>kern/pmap.c</code> 中的 <code>mem_init()</code> 函数来 <strong>分配</strong> 并 <strong>映射</strong> <code>envs</code> 数组。这个数组恰好包含 <code>NENV</code> 个 <code>Env</code> 结构体实例，这与你分配 <code>pages</code> 数组的方式非常相似。另一个相似之处是，支持 <code>envs</code> 的内存储应该被只读映射在页表中 <code>UENVS</code> 的位置（于 <code>inc/memlayout.h</code> 中定义），所以，用户进程可以从这一数组读取数据。</p>
    <p>修改好后，<code>check_kern_pgdir()</code> 应该能够成功执行。</p></section>
  ...
  <section class="custom-block exercise" type="exercise"><strong>练习 2.</strong><p>在 <code>env.c</code> 中，完成接下来的这些函数：</p>
    <ul>
      <li>
    ...
    <p>当你在完成这些函数时，你也许会发现 cprintf 的新的 <code>%e</code> 很好用，它会打印出与错误代码相对应的描述，例如： <code>r = -E_NO_MEM; panic("env_alloc: %e", r);</code> 会 panic 并打印出 <code>env_alloc: out of memory</code>。</p></section>
  ...
  <section class="custom-block exercise" type="exercise"><strong>练习 4.</strong><p>编辑 <code>trapentry.S</code> 和 <code>trap.c</code>，以实现上面描述的功能。 <code>trapentry.S</code> 中的宏定义 <code>TRAPHANDLER</code> 和 <code>TRAPHANDLER_NOEC</code>，还有在 <code>inc/trap.h</code> 中的那些 <code>T_</code> 开头的宏定义应该能帮到你。你需要在 <code>trapentry.S</code> 中用那些宏定义为每一个 <code>inc/trap.h</code> 中的 trap (陷阱) 添加一个新的入口点，你也要提供 <code>TRAPHANDLER</code> 宏所指向的 <code>_alltraps</code> 的代码。你还要修改 <code>trap_init()</code> 来初始化 <code>IDT</code>，使其指向每一个定义在 <code>trapentry.S</code> 中的入口点。<code>SETGATE</code> 宏定义在这里会很有帮助。<br>
    ...
    <p>用一些 <code>user</code> 目录下会造成异常的程序测试一下你的陷阱处理代码，比如 <code>user/divzero</code>。现在，你应该能在 <code>make grade</code> 中通过 <code>divzero</code>, <code>softint</code> 和 <code>badsegment</code> 了。</p></section>
  <section class="custom-block challenge" type="challenge"><strong>挑战！</strong><p>现在，无论是在 <code>trapentry.S</code> 中的 <code>TRAPHANDLER</code>，或者是配置它们的 <code>trap.c</code>中，你也许写了太多非常相似的代码了。试着整理一下。调整一下 <code>trapentry.S</code> 中的宏定义，让它自动生成一个给 <code>trap.c</code> 使用的表。注意，你可以在汇编中通过 <code>.text</code> 和 <code>.data</code> <a href="https://en.wikipedia.org/wiki/Directive_(programming)">(这是什么？)</a> 来随时在代码段和数据段切换。</p></section>
  <section class="custom-block question" type="question"><strong>问题</strong><p>在 <code>answers-lab3.txt</code> 中回答下面这些问题：</p>
    <ul>
  ...
  <section class="custom-block exercise" type="exercise"><strong>练习 6.</strong><p>修改 <code>trap_dispatch()</code> 使断点异常唤起内核监视器。现在，你应该能够让 <code>make grade</code> 在 <code>breakpoint</code> 测试中成功了。</p></section>
  <section class="custom-block challenge" type="challenge"><strong>挑战！</strong><p>修改你的 JOS 内核，让你能够在断点之后从当前位置恢复运行，或者在断点之后继续单步运行。你需要理解 <code>EFLAGS</code> 中的某个特定的位来实现单步运行。</p></section>
  <section class="custom-block challenge" type="challenge"><strong>可选</strong><p>如果你非常热爱挑战，试着找一些 x86 反汇编代码，比如，从 QEMU 中拿到它，或者从 GNU binutils 中找找，或者自己写一些。拓展 JOS 内核监视器，使其能够反汇编并显示你正在单步执行的指令。结合我们在 lab 2 中实现的符号表，这些事情就是真正的内核调试器所做的了。</p></section>
  ...
  <section class="custom-block exercise" type="exercise"><strong>练习 7.</strong><p>在内核中断描述符表中为中断向量 <code>T_SYSCALL</code> 添加一个处理函数。你需要编辑 <code>kern/trapentry.S</code> 和 <code>kern/trap.c</code> 的 <code>trap_init()</code> 方法。你也需要修改 <code>trap_dispath()</code> 来将系统调用中断分发给在 <code>kern/syscall.c</code> 中定义的 <code>syscall()</code>。确保如果系统调用号不合法，<code>syscall()</code> 返回 <code>-E_INVAL</code>。你应该读一读并且理解 <code>lib/syscall.c</code>（尤其是内联汇编例程）来确定你已经理解了系统调用接口。通过调用相应的内核函数，处理在 <code>inc/syscall.h</code> 中定义的所有系统调用。</p>
    <p>通过 <code>make run-hello</code> 运行你的内核下的 <code>user/hello</code> 用户程序，它现在应该能在控制台中打印出 <strong>hello, world</strong> 了，接下来会在用户模式造成一个缺页。如果这些没有发生，也许意味着你的系统调用处理函数不太对。现在应该也能在 <code>make grade</code> 中通过 <code>testbss</code> 这个测试了。</p></section>
  <section class="custom-block challenge" type="challenge"><strong>挑战！</strong><p>通过使用 <code>sysenter</code> 和 <code>sysexit</code> 指令实现系统调用，而不是 <code>int 0x30</code> 和 <code>iret</code>。</p>
    ...
    <p>注意这样做只能支持 4 个参数，所以你需要把旧的系统调用方式保留下来来支持那些有 5 个参数的系统调用。而且，因为这个快速路径不会更新当前进程的陷阱帧，所以它也不适合我们接下来要加入的一些系统调用。<br>
      一旦我们在下个实验中启用异步中断，你也许需要再回顾一下你的代码。具体来说，你应该需要在返回用户进程时启用中断，<code>sysexit</code> 不会帮你这么做。</p></section>
  ...
  <section class="custom-block exercise" type="exercise"><strong>练习 8.</strong><p>在用户库文件中补全所需要的代码，并启动你的内核。你应该能看到 <code>user/hello</code> 打出了 <code>hello, world</code> 和 <code>i am environment 00001000</code>。接下来，<code>user/hello</code> 尝试通过调用 <code>sys_env_destory()</code> 方法退出（在 <code>lib/libmain.c</code> 和 <code>lib/exit.c</code>）。因为内核目前只支持单用户进程，它应该会报告它已经销毁了这个唯一的进程并进入内核监视器。在这时，你应该能够在 <code>make grade</code> 中通过 <code>hello</code> 这个测试了。</p></section>
  ...
  <section class="custom-block exercise" type="exercise"><strong>练习 9.</strong><p>修改 <code>kern/trap.c</code>，如果缺页发生在内核模式，应该恐慌。</p>
